...
固定最佳 Worker=1、Release、同一语料、同一查询、`wrk -t4 -d30s --latency`，只将连接数从 64 提升到 256：
...
| 连接数 | QPS | 平均延迟 | P99 | CPU | RSS |
|---:|---:|---:|---:|---:|---:|
| 64 | 142,430.82 | 453.60us | 0.86ms | 175.81% | 3344KB |
| 256 | 145,068.36 | 1.78ms | 2.44ms | 172.00% | 3944KB |

连接数增加四倍后，QPS 仅增加约 1.9%，CPU 未继续增长，但平均延迟约增至 3.9 倍、P99 约增至 2.8 倍。这说明在 `c64` 左右已经接近当前服务能力上限；额外连接主要在请求队列、Worker 和 EventLoop 的串行写回环节等待，而非转换为更多有效吞吐。

对于当前轻量检索负载，推荐默认使用 **1 Worker**，并将压测平台点设为 **64 并发连接**。生产场景仍需结合真实查询复杂度、文档规模、CPU 核数与目标 P99 重新测量，不能把本机回环结果视为通用容量上限。

## c256 下的 Worker 再次对比与线程级 CPU（2026-08-18）

固定 `c256` 后重新比较 Worker 数量：

| Worker | QPS | P99 | 观察 |
|---:|---:|---:|---|
| 1 | 126,818.10 / 114,608.91 | 3.37ms / 3.34ms | 单 Worker 开始出现业务排队 |
| 2 | 131,497.59 / 125,108.72 | 2.39ms / 2.40ms | 高连接数下的较优点 |
| 4 | 125,730.34 | 2.85ms | 继续增加无收益 |
| 8 | 98,184.57 | 3.46ms | 同步与调度成本显著 |

高连接数下，Worker=2 相比 Worker=1 有约 4%-9% 吞吐收益，并显著降低 P99，说明单 Worker 的任务队列确实有部分积压；但增加到 4、8 后再次退化。需要注意：Worker=2 的 c256 吞吐仍低于 Worker=1 的 c64 平台点，因此“增大连接数 + 增加 Worker”不是当前服务的最优方案。

### 线程级监控命令

```bash
PID=<服务 PID>
pidstat -t -p "$PID" -u 1 31
```

`-t` 会按线程（TID）报告 CPU。当前进程的主线程 TID 与 PID 相同，运行 EventLoop；后续两个 TID 是 WorkerPool 创建的 Worker。

Worker=2、c256 的一次监控轮（136,799.38 QPS、P99 2.11ms）：

| 线程角色 | %usr | %system | %CPU | 含义 |
|---|---:|---:|---:|---|
| EventLoop 主线程 | 23.93 | 72.30 | 96.23 | 接近单核上限，主要消耗在内核网络/I/O 路径 |
| Worker 1 | 53.79 | 5.80 | 59.59 | 主要执行用户态检索与 JSON 构造 |
| Worker 2 | 53.76 | 5.03 | 58.79 | 同上 |
| 进程总计 | 131.47 | 83.17 | 214.64 | 三条线程的总和 |

这能支持“EventLoop 是下一阶段重点观察对象”的判断，但还不能仅凭 CPU 断言其具体热点。下一步若需继续优化，应使用 `perf` 等采样工具确认 EventLoop 的时间具体花在 `epoll_wait`、`recv/send`、锁、eventfd 还是用户态回调上，再决定是否引入多 Reactor。
